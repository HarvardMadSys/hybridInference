...
class DatabaseLogger:
    ...
    async def upsert_request(
        ...
        """Insert or update a request log entry (UPSERT).

        This method ensures the request is logged regardless of whether a prior
        pending insert succeeded. Uses INSERT ... ON CONFLICT DO UPDATE.
        ...
        Args:
            ...
            usage: Token usage breakdown.
            latency_ms: End-to-end latency in milliseconds.
            status_code: HTTP status code (0=pending, 200=success, 500=error, -1=cancelled).
            error: Optional error message.
            params: Request parameters (temperature, max_tokens, etc.).
            metadata: Additional metadata.
            ttft_ms: Time to first token in milliseconds.
            pricing: Model pricing config for cost calculation.
        """
        if not self.pool:
            raise RuntimeError("DatabaseLogger not initialized")
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                ON CONFLICT (request_id) DO UPDATE SET
                    ...
                    metadata = COALESCE(api_logs.metadata, '{}'::jsonb) || COALESCE(EXCLUDED.metadata, '{}'::jsonb)
                """,
                request_id,
                model_id,
                provider,
