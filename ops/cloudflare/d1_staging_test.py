...
Usage:
    ...
    python ops/cloudflare/d1_staging_test.py --store-only

    # Part 2: Full HTTP flow (staging server must be running with DB_BACKEND=d1)
    python ops/cloudflare/d1_staging_test.py --http-only --base-url https://staging.freeinference.org

    # Both
    python ops/cloudflare/d1_staging_test.py --base-url https://staging.freeinference.org

    # Clean up test data after run
    python ops/cloudflare/d1_staging_test.py --cleanup
