...
- **A foreign owner label, or a different container id, demotes the backend to `stopped`** and releases its GPU bookkeeping. Nothing is removed: the container is its new owner's and may be mid-load or mid-stream, which is the conclusion the idle path already reaches for the same situation. The next request re-decides ownership from scratch and 502s naming both owners, so the gateway can fail over. The id matters on its own because a takeover can read as *ours*: an operator hand-restarting a wedged backend leaves an unlabelled container, which the upgrade rule below deliberately reads as this proxy's own, and two units can be given the same `PROXY_OWNER`.
