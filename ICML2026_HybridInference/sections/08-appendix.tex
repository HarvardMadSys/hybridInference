...
\begin{itemize}
    ...
    \item \textbf{Smart Hedge}: LP-Mix as primary routing, with cost-benefit hedging (Eq.~\ref{eq:hedge_economic}) that triggers a backup request to the fastest provider when the expected benefit of avoiding a violation exceeds the backup cost ($C_b/V = 0.05$).
\end{itemize}
...
\begin{itemize}
    ...
    \item The cost-benefit rule triggers selectively on 15.6\% of requests---only when the expected benefit of avoiding a violation exceeds the backup cost.
\end{itemize}
...
\paragraph{Alternative: Survival Comparison Rule.}
A simpler alternative compares the primary's conditional violation probability against the backup's:
\[
\frac{S_p(L)}{S_p(t)} > S_b(L - t - \delta)
\]
This triggers hedging when waiting is riskier than hedging. However, when the backup is very fast (e.g., Groq), the right side approaches 0, making the condition nearly always true---leading to hedge rates of 28--87\% depending on backup provider. The cost-benefit rule~\eqref{eq:hedge_economic} avoids this by incorporating backup cost, which naturally penalizes over-hedging.

\paragraph{Alternative: Residual Life Decision Rule.}
Another alternative uses expected residual life. Given elapsed time $t$, hedge when:
\[
t + \mathbb{E}[\text{Remaining} \mid T > t] + \delta + \mathbb{E}[T_{\text{backup}}] > L_{\text{SLO}}
\]
This was originally designed for cancel-and-resend (serial) execution, where the backup runs \textit{after} the primary is cancelled. Our system uses parallel hedging (both run concurrently), making the serial formula overly conservative---it double-counts time by including $\mathbb{E}[T_{\text{backup}}]$ as an additive term rather than modeling the min of two concurrent processes.

\paragraph{Hedging Strategy Comparison.}
Table~\ref{tab:hedging_strategies} compares hedging strategies in simulation (SLO = 2s, 500 evaluation points, backup = fastest):
\begin{table}[h]
...
\begin{tabular}{lcccc}
\toprule
Strategy & Hedge Rate & Viol. Rate & P99 (ms) & Description \\
\midrule
Never & 0\% & 1.4\% & 2314 & LP-mix only \\
Always & 100\% & 0\% & 547 & Send two requests always \\
Fixed-0.5 & 5.2\% & 0\% & 1425 & Hedge at $0.5 \cdot L$ \\
Survival & 15.2\% & 0\% & 1328 & $S_p(L)/S_p(t) > S_b$ \\
Residual & 3.6\% & 0.6\% & 1495 & Serial formula \\
\textbf{Economic} & \textbf{8.6\%} & \textbf{0\%} & \textbf{1328} & $P_\text{viol} \cdot F_b > C_b/V$ \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

The economic rule achieves zero violations at a moderate hedge rate (8.6\%), with the cost ratio $C_b/V$ providing a single, economically interpretable knob for controlling the cost-violation tradeoff.
