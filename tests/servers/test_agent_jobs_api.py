...
@pytest.mark.parametrize("source_state", ["running", "cancelled"])
async def test_restart_copies_the_source_configuration_into_a_new_root(
    client: AsyncClient, store: FakeAgentJobStore, source_state: str
):
    """Editing the initial prompt keeps its environment but starts fresh history."""
    source_response = await client.post(
        "/v1/agent/jobs",
        json={
            "repo": "owner/name",
            "task_prompt": "original direction",
            "runtime": "codex",
            "model": "glm-5.1",
    ...
    source = store.jobs[source_response.json()["id"]]
    source["state"] = source_state

    response = await client.post(
        f"/v1/agent/jobs/{source['id']}/restart",
        json={"prompt": "  corrected direction  "},
    )

    assert response.status_code == 201
    restarted = response.json()
    restarted_row = store.jobs[restarted["id"]]
    assert restarted["id"] != source["id"]
    assert restarted["thread_id"] != source["thread_id"]
    assert restarted["parent_job_id"] is None
    assert restarted["turn_no"] == 1
    assert restarted["task_prompt"] == "corrected direction"
    assert restarted["repo"] == source["repo"]
    assert restarted["base_sha"] == source["base_sha"]
    assert restarted["base_ref"] == "dev"
    assert restarted["runtime"] == source["runtime"]
    assert restarted["model"] == source["model"]
    assert restarted["budget_usd"] == source["budget_usd"]
    assert restarted["metadata"] == {"ticket": "ABC-1"}
    assert restarted_row["setup_script"] == "uv sync"
    assert source["state"] == source_state
    assert source["cancel_requested"] is False


async def test_restart_is_owner_scoped(client: AsyncClient, store: FakeAgentJobStore):
    """A foreign source id remains indistinguishable from a missing job."""
    source_id = await _create_job(client)
    store.jobs[source_id]["user_id"] = _OTHER

    response = await client.post(
        f"/v1/agent/jobs/{source_id}/restart", json={"prompt": "try again"}
    )

    assert response.status_code == 404


async def test_restart_rechecks_model_and_repository_access(
    client: AsyncClient, store: FakeAgentJobStore
):
    """Old task ownership does not bypass current model or repo entitlement."""
    source_id = await _create_job(client)
    store.jobs[source_id]["model"] = "gpt-no-longer-available"

    unavailable_model = await client.post(
        f"/v1/agent/jobs/{source_id}/restart", json={"prompt": "try again"}
    )

    assert unavailable_model.status_code == 400
    assert unavailable_model.json()["detail"]["error"]["type"] == "model_not_available"

    store.jobs[source_id]["model"] = "glm-5.1"
    store.jobs[source_id]["repo"] = "someone-else/private"
    unentitled_repo = await client.post(
        f"/v1/agent/jobs/{source_id}/restart", json={"prompt": "try again"}
    )
    assert unentitled_repo.status_code == 403
    assert unentitled_repo.json()["detail"]["error"]["type"] == "repo_not_allowed"
