...
export interface AgentRunnerHostsResponse {
  hosts: AgentRunnerHost[];
  active_host: string | null;
}

export async function getAgentRunnerHosts(): Promise<AgentRunnerHostsResponse> {
  const resp = await fetchWithAuth(API_BASE, '/admin/agent/runner-hosts');
  return jsonOrThrow<AgentRunnerHostsResponse>(resp);
}

/** Pin agent jobs to one host, or pass null to let any runner claim. */
export async function setActiveAgentRunnerHost(
  host: string | null,
): Promise<AgentRunnerHostsResponse> {
  const resp = await fetchWithAuth(API_BASE, '/admin/agent/runner-hosts/active', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function forgetAgentRunnerHost(host: string): Promise<AgentRunnerHostsResponse> {
  const resp = await fetchWithAuth(
    API_BASE,
    `/admin/agent/runner-hosts/${encodeURIComponent(host)}`,
    { method: 'DELETE' },
  );
  return jsonOrThrow<AgentRunnerHostsResponse>(resp);
}
