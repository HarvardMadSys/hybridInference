...
def create_app() -> FastAPI:
    ...
    # ``input`` back at them (a password or an API key, on the auth and admin
    # routers). Same 422 status; the replacement publishes the ``loc`` field
    # names, which tell a client what to fix, and drops ``input``, which only
    # tells them what they already sent. Full detail to the log. Registered here
    # for the same reason as the pair above: last registration wins, and this
