...
def fig_decode_vs_input(datasets: dict[str, list[dict]]) -> None:
    ...
    for key, data in datasets.items():
        ...
        for in_tok in INPUT_LENGTHS:
            pt = next(
                (r["prompt_tokens_avg"] for r in data if r["input_tokens_target"] == in_tok), in_tok
            )
    ...
    ax.set_title(
        "Decode Throughput vs Prompt Length\n(averaged over output lengths 64-512)",
        fontsize=13,
    )
        ...
        ys = [
            next(
                (
                    r["prefill_ms"]
                    for r in data
                    if r["input_tokens_target"] == i and r["output_tokens_target"] == 256
                ),
                0.0,
            )
...
def fig_decode_vs_output(datasets: dict[str, list[dict]]) -> None:
    ...
    for (key, data), offset in zip(datasets.items(), offsets, strict=True):
        ys = [
            next(
                (
                    r["decode_throughput_tok_s"]
                    for r in data
                    if r["input_tokens_target"] == 1024 and r["output_tokens_target"] == o
                ),
                0.0,
            )
            for o in OUTPUT_LENGTHS
        ]
        bars = ax.bar(x + offset, ys, width, label=LABELS[key], color=COLORS[key], alpha=0.85)
        for bar in bars:
            h = bar.get_height()
            ax.annotate(
                f"{h:.0f}",
...
def fig_e2e_latency_bars(datasets: dict[str, list[dict]]) -> None:
    ...
    for ax, out_tok in zip(axes, OUTPUT_LENGTHS, strict=True):
        ys = [
            next(
                (
                    r["total_latency_ms"] / 1000
                    for r in datasets[k]
                    if r["input_tokens_target"] == 1024 and r["output_tokens_target"] == out_tok
                ),
                0.0,
            )
            for k in keys
    ...
    fig.legend(
        handles, bar_labels, loc="lower center", ncol=3, fontsize=10, bbox_to_anchor=(0.5, -0.08)
    )
    fig.tight_layout()
...
def fig_sglang_vs_vllm(qwen_sg: list[dict], qwen_vl: list[dict]) -> None:
    ...
    ax1.plot(
        ...
        dec_sg,
        "o-",
        color=COLORS["qwen_sglang"],
        linewidth=2.2,
        markersize=8,
        label=LABELS["qwen_sglang"],
    )
    ax1.plot(
        pt_vl,
        dec_vl,
        "s-",
        color=COLORS["qwen_vllm"],
        linewidth=2.2,
        markersize=8,
        label=LABELS["qwen_vllm"],
    )
