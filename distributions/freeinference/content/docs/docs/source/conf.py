...
def _find_repository_root() -> Path:
    """Find the checkout root from either transitional documentation tree."""
    for candidate in Path(__file__).resolve().parents:
        if (candidate / "pyproject.toml").is_file() and (
            candidate / "ops/setup/setup_claude_code.sh"
        ).is_file():
            return candidate
    raise RuntimeError("Could not locate the repository root from the Sphinx configuration")
...
html_extra_path = [str(_find_repository_root() / "ops/setup/setup_claude_code.sh")]
