...
> **⛔ 已终结（2026-07-30）：本文档的步骤 4–8（后端迁移、production 上线、legacy
> 删除)已按 issue #1103 的决议取消,不再执行。**终态:status-monitor 三类告警
> 保留在控制平面上(它的正式告警栈);后端网关永久保留 `alert_slack` webhook 路
> 径,并已获得恢复边(#1076)。本文档仅作历史记录,后续以 #1103 为准。
>
