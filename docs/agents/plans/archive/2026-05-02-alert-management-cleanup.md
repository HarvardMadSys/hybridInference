...
**Architecture:** The internal `deploy/alertmanager/` config gains a Slack receiver mirroring the external monitor's, gated by an explicit `match_re` allowlist of vetted alert names. The `alert-logger` webhook (`alert_logger.py` + systemd unit) is removed. Empty rule files (`pipeline_health.yml`, `slo_burn_rate.yml`) are deleted, with a short README in `deploy/prometheus/rules/` recording the removal and pointing to git history. Webhook URL injection uses `envsubst` at deploy time, matching the existing external-monitor pattern. The external monitor stack is untouched.

**Tech Stack:** Prometheus 2.x, Alertmanager 0.28, Slack incoming webhooks, systemd, `envsubst` (gettext-base).

**Spec:** `docs/agents/specs/2026-05-02-alert-management-cleanup-design.md`
...
- `deploy/alertmanager/README.md` documents the file
...
- `docs/developer/developer/architecture.md` had a spaced "alert logger"
  mention (also originally missed by the plan's hyphen-only grep) which
  was cleaned up.
- `deploy/prometheus/README.md` dropped the "Slack/Email"
...
**Modify:**
- `deploy/alertmanager/alertmanager.yml` — replace `alert-logger` receiver with `slack-default`; keep allowlist routing.
- `deploy/alertmanager/alertmanager.yml.example` — drop email block; Slack-only template.
- `deploy/alertmanager/README.md` — update for single Slack receiver; drop alert-logger references; document `envsubst` step.
- `deploy/prometheus/prometheus.yml` — remove `rules/slo_burn_rate.yml` and `rules/pipeline_health.yml` from `rule_files`; add `global.external_labels.source: internal`.
- `docs/developer/developer/deployment.md` — remove `alert-logger` from infra list and flow diagram.
- `docs/developer/developer/freeinference.md` — drop `alert-logger` from infra list.

**Delete:**
- `services/alert-logger/alert_logger.py`
- `deploy/systemd/alert-logger.service`
- `deploy/prometheus/rules/pipeline_health.yml`
- `deploy/prometheus/rules/slo_burn_rate.yml`

**Create:**
- `deploy/prometheus/rules/README.md` — record removal of SLO/pipeline-health rules + reasons.
...
**Files:**
- Modify: `deploy/alertmanager/alertmanager.yml`

- [ ] **Step 1: Verify current content**

Run:
```bash
cat deploy/alertmanager/alertmanager.yml
```
...
Overwrite `deploy/alertmanager/alertmanager.yml` with:
...
SLACK_WEBHOOK_URL='https://hooks.slack.com/services/PLACEHOLDER' \
  envsubst < deploy/alertmanager/alertmanager.yml > /tmp/am-check.yml
...
```bash
git add deploy/alertmanager/alertmanager.yml
...
```bash
sed -n '1,10p' deploy/prometheus/prometheus.yml
```
...
Edit `deploy/prometheus/prometheus.yml`. Replace:
...
**Files:**
- Delete: `deploy/prometheus/rules/pipeline_health.yml`
- Delete: `deploy/prometheus/rules/slo_burn_rate.yml`

- [ ] **Step 1: Confirm both files contain only comments + empty `rules: []`**

```bash
grep -v -E '^\s*(#|$)' deploy/prometheus/rules/pipeline_health.yml
grep -v -E '^\s*(#|$)' deploy/prometheus/rules/slo_burn_rate.yml
```
...
```bash
git rm deploy/prometheus/rules/pipeline_health.yml
git rm deploy/prometheus/rules/slo_burn_rate.yml
```
...
as noisy). Removal documented in deploy/prometheus/rules/README.md
(added in next commit). Original rule definitions remain in git history."
```

---

## Task 4: Add `deploy/prometheus/rules/README.md` documenting removal

**Files:**
- Create: `deploy/prometheus/rules/README.md`

- [ ] **Step 1: Create the README**

Write `deploy/prometheus/rules/README.md`:
...
```bash
git log --all --oneline -- deploy/prometheus/rules/pipeline_health.yml
git log --all --oneline -- deploy/prometheus/rules/slo_burn_rate.yml
```

## Adding a new rule

1. Add or edit a `*.yml` file in this directory.
2. Reference it under `rule_files:` in `deploy/prometheus/prometheus.yml`.
3. To page on the new alert, add the `alertname` to the `match_re`
   allowlist in `deploy/alertmanager/alertmanager.yml`.
4. Validate with `promtool check config deploy/prometheus/prometheus.yml`.
```

- [ ] **Step 2: Commit**

```bash
git add deploy/prometheus/rules/README.md
...
**Files:**
- Delete: `services/alert-logger/alert_logger.py`
- Delete: `deploy/systemd/alert-logger.service`
...
```bash
git rm services/alert-logger/alert_logger.py
git rm deploy/systemd/alert-logger.service
```
...
**Files:**
- Modify: `deploy/alertmanager/alertmanager.yml.example`

- [ ] **Step 1: Replace file contents**

Overwrite `deploy/alertmanager/alertmanager.yml.example` with:

```yaml
# Template for deploy/alertmanager/alertmanager.yml.
# Copy to alertmanager.yml and replace ${SLACK_WEBHOOK_URL} at deploy
# time (envsubst recommended; see deploy/alertmanager/README.md).
...
```bash
git add deploy/alertmanager/alertmanager.yml.example
git commit -m "docs(alertmanager): trim example template to Slack-only with allowlist"
```

---

## Task 7: Rewrite `deploy/alertmanager/README.md`

**Files:**
- Modify: `deploy/alertmanager/README.md`

- [ ] **Step 1: Replace file contents**

Overwrite `deploy/alertmanager/README.md` with:
...
in `deploy/external-monitor/` and runs its own Alertmanager.
...
set +a
envsubst < /srv/hybridInference/deploy/alertmanager/alertmanager.yml \
...
1. Define the rule in `deploy/prometheus/rules/`.
...
- Synthetic test: temporarily lower a `for: 2m` rule to `for: 10s` in
  `deploy/prometheus/rules/service_availability.yml`, stop the
  app process, confirm a Slack message arrives. Revert the rule.

## Related

- Prometheus rules: `deploy/prometheus/rules/`.
- External monitor: `deploy/external-monitor/`.
```

- [ ] **Step 2: Commit**

```bash
git add deploy/alertmanager/README.md
...
**Files:**
- Modify: `docs/developer/developer/deployment.md`
- Modify: `docs/developer/developer/freeinference.md`

- [ ] **Step 1: View current `alert-logger` mentions**

```bash
grep -n "alert-logger" docs/developer/developer/deployment.md docs/developer/developer/freeinference.md
```

Expected matches:
- `docs/developer/developer/deployment.md:23`
- `docs/developer/developer/deployment.md:42`
- `docs/developer/developer/freeinference.md:27`

- [ ] **Step 2: Edit `docs/developer/developer/deployment.md`**

Read line 23 context first:
```bash
sed -n '20,26p' docs/developer/developer/deployment.md
```
...
```bash
sed -n '40,46p' docs/developer/developer/deployment.md
```
...
- [ ] **Step 3: Edit `docs/developer/developer/freeinference.md`**

Read line 27 context:
```bash
sed -n '25,29p' docs/developer/developer/freeinference.md
```
...
```bash
git add docs/developer/developer/deployment.md docs/developer/developer/freeinference.md
...
```bash
grep -E "^\s*- alert:" deploy/prometheus/rules/service_availability.yml | awk '{print $3}'
grep -A1 "match_re:" deploy/alertmanager/alertmanager.yml
```
...
- Deleted fully-commented-out `pipeline_health.yml` and `slo_burn_rate.yml` rule files (noisy since 2026-02-11). New `deploy/prometheus/rules/README.md` records why and points to git history.
- Added `external_labels.source: internal` on internal Prometheus to disambiguate against the external stack.
- Webhook URL injection uses deploy-time `envsubst`, matching the existing external-monitor pattern. Documented in the rewritten `deploy/alertmanager/README.md`.
...
- Add `deploy/prometheus/rules/README.md` → Task 4.
