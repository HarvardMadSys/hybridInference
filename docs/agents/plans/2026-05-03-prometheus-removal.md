...
**Spec:** [docs/agents/specs/2026-05-03-prometheus-removal-design.md](../specs/2026-05-03-prometheus-removal-design.md)
...
- `deploy/alertmanager/` (entire directory, includes `alert_logger.py`, `alertmanager.yml`, `alertmanager.yml.example`)
- `deploy/systemd/alertmanager.service`
- `deploy/systemd/alert-logger.service`
- `deploy/docker/Dockerfile.alert-logger`

### Modified files (Phase 3)

| File | Change |
|---|---|
| `deploy/docker/docker-compose.yml` | Remove `alertmanager` and `alert-logger` services (lines ~130-160); remove `alertmanager_data` and `alert_log_data` volumes (lines ~162-173). |
| `Makefile` | Remove any prometheus/alertmanager targets if present. |
| Docs in `docs/developer/` | Remove Prometheus/Alertmanager references; add observability section pointing at the new framework. |
...
Body: link the spec at `docs/agents/specs/2026-05-03-prometheus-removal-design.md`. Note this is Phase 1 of 3 PRs; Phase 1 builds the framework dark.
...
Spec: docs/agents/specs/2026-05-03-prometheus-removal-design.md
Plan: docs/agents/plans/2026-05-03-prometheus-removal.md
...
### Task 31: Delete `deploy/prometheus/`
...
- [ ] **Step 3: Commit** `chore(infra): delete deploy/prometheus/`

---

### Task 32: Delete `deploy/alertmanager/`

- [ ] Confirm no references via grep, then `rm -rf infrastructure/alertmanager`
- [ ] Commit: `chore(infra): delete deploy/alertmanager/`
...
```bash
rm deploy/systemd/alertmanager.service
rm deploy/systemd/alert-logger.service
```
...
### Task 34: Update `deploy/docker/docker-compose.yml`
...
### Task 35: Delete `deploy/docker/Dockerfile.alert-logger`

```bash
rm deploy/docker/Dockerfile.alert-logger
```
...
- [ ] **Step 2: Remove or rewrite each match.** In `docs/developer/developer/`, add a short observability section pointing at `serving/observability/` and `config/alerts.yaml`.
...
- [ ] `deploy/prometheus/` and `deploy/alertmanager/` no longer exist.
...
- [ ] `docker compose -f deploy/docker/docker-compose.yml config` parses cleanly with no `alertmanager` / `alert-logger` references.
