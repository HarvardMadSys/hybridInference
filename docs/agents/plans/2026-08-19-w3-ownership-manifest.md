...
`/srv/freeInference/ops/db/` 执行,staging 主机 `spark_idle_proxy.service` 指向
...
移植);(c) 行 46 的"主机 systemd 单元指向新路径"验证:staging 主机 已验,
其他 GPU 主机 无 shell 待 W7 清点(上游 git 删除不影响在跑主机)。
