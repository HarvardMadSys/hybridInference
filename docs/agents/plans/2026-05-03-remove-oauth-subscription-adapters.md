...
**Spec:** `docs/agents/specs/2026-05-03-remove-oauth-subscription-adapters-design.md` (commit it as the first commit on the implementation branch).
...
- **Static grep:** `git grep -nI 'codex_sub\|codex_translator\|codex_token\|claude_sub\|claude_token\|claude_pool\|ClaudeSubscriptionAdapter\|CodexSubscriptionAdapter\|import_codex_auth\|import_claude_auth\|inspect_claude_accounts\|CODEX_BASE_URL' -- ':!docs/agents/' ':!test/unit/routing/test_routewise_router.py'`
...
- Move into branch: `docs/agents/specs/2026-05-03-remove-oauth-subscription-adapters-design.md`
...
```bash
ls -la docs/agents/specs/2026-05-03-remove-oauth-subscription-adapters-design.md
```
...
See spec: `docs/agents/specs/2026-05-03-remove-oauth-subscription-adapters-design.md` (will land in the implementation PR).

Scope: 15 file deletes + 22 file edits.

Out of scope:
- `cli-proxy` upstream and `${CLI_PROXY_*}` env vars (live Claude path)
- `ops/setup/setup_claude_code.sh` (end-user setup)
...
```bash
git grep -n 'codex_accounts_file\|codex_fallback_api_key\|codex_token_refresh_margin\|codex_account_cooldown\|codex_failure_threshold\|claude_sub_accounts_file\|claude_sub_fallback_api_key\|claude_sub_token_refresh_margin\|claude_sub_account_cooldown\|claude_sub_failure_threshold' -- ':!docs/agents/'
```
...
- Modify: `docs/user/docs/developer/integrations.md`
- Modify: `docs/user/docs/developer/quickstart.md`
- Modify: `docs/user/docs/developer/models.md`
- Modify: `docs/developer/developer/architecture.md`
- Modify: `docs/developer/developer/configuration.md`
- Modify: `docs/developer/developer/adding-models.md`
- Modify: `docs/developer/developer/openrouter.md`
- Modify: `docs/developer/developer/deployment.md`

- [ ] **Step 1: `docs/user/README.md` — drop Codex bullet and Codex Setup section**

In `docs/user/README.md`:
...
- [ ] **Step 2: `docs/user/docs/developer/integrations.md` — strip the entire Codex section**

In `docs/user/docs/developer/integrations.md`:
...
- [ ] **Step 3: `docs/user/docs/developer/quickstart.md` — drop Codex Setup step**

In `docs/user/docs/developer/quickstart.md`, delete the "### Codex" subsection (lines ~22–44, from `### Codex` through `[Detailed setup →](integrations.md)` and the blank line after). The "### Roo Code / Kilo Code" section should follow "### Cursor" directly.

- [ ] **Step 4: `docs/user/docs/developer/models.md` — drop GPT-5.4 entry and Codex switcher block**

In `docs/user/docs/developer/models.md`:
...
- [ ] **Step 5: `docs/developer/developer/architecture.md` — drop Subscription Adapters section and stale adapter mentions**

In `docs/developer/developer/architecture.md`:
...
- [ ] **Step 6: `docs/developer/developer/configuration.md` — drop subscription kinds and §5.4**

In `docs/developer/developer/configuration.md`:
...
(c) Check the surrounding context — if there's a `### 5.x Claude Subscription` subsection, delete that too. (Run `grep -n "Claude Subscription\|claude_sub" docs/developer/developer/configuration.md` to identify any remaining content, then remove.)

- [ ] **Step 7: `docs/developer/developer/adding-models.md` — drop subscription rows**

In `docs/developer/developer/adding-models.md`:
...
- [ ] **Step 8: `docs/developer/developer/openrouter.md` — drop codex_sub.py from the code-tree comment**

In `docs/developer/developer/openrouter.md`, edit line 18 from:
...
If the surrounding lines also reference `claude_sub.py`, scrub them too. Run `grep -n 'claude_sub\|codex_sub' docs/developer/developer/openrouter.md` to confirm clean.

- [ ] **Step 9: `docs/developer/developer/deployment.md` — strip subscription account management section**

In `docs/developer/developer/deployment.md`, delete the entire `## Subscription Account Management` section. It runs from line ~103 (`## Subscription Account Management`) through the end of the "Verify Anthropic-native surface" subsection (which depends on subscription routing). Specifically, delete:
...
```bash
grep -n '^## ' docs/developer/developer/deployment.md
```
...
```bash
git add docs/user/ docs/developer/developer/
git commit -m "$(cat <<'EOF'
...
- `docs/agents/` is historical — never edit.
...
```bash
git grep -nI 'gpt-5\.4\|gpt-5\.3-codex\|gpt-5\.2-codex\|gpt-5\.1-codex\|GPT-5\.4' | grep -v '^docs/agents/'
```
...
- `ops/setup/setup_claude_code.sh` stays (configures end-user Claude Code CLI to point at `freeinference.org/anthropic`; unrelated to OAuth pool).
