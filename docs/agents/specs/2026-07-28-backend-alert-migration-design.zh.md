...
> **⛔ 已废弃（2026-07-30）：本迁移已按 issue #1103 的决议终止,不再实施。**
> 后端网关永久保留 `alert_slack` webhook 路径;本文档定案的 G2 两个契约类型
> (`metric_threshold_breach` / `dependency_unavailable`)与 producer 侧代码已
> 从仓库删除(删除 PR 即恢复点)。G4 发现的恢复语义问题由 #1076 在 legacy 路
> 径上独立解决。本文档仅作历史记录——其中 G1 的部署期长期令牌方案是将来若重
> 启时的候选身份方案。
>
...
**`alert_slack` 的返回值不足以判断"事件已被控制平面接收"。**

它在 relay 与 webhook 中任一成功时就返回 `True`。今天两者都只是人看的通道，
所以这没问题。接上控制平面之后不成立 —— 若 relay（持有 incident 生命周期的
那个 sink）失败而 webhook 兜底成功，`alert_slack` 仍返回 `True`,于是
`alert_on_transition` 不会重挂,而控制平面**从未收到那条恢复**,incident 永久
挂着。

这是"某个 sink 成功"与"拥有生命周期的那个 sink 成功"被混为一谈。4.3 接传输时
必须让重挂的判据变成后者 —— 例如让 `alert_slack` 返回按 sink 区分的结果,而不是
一个布尔。

在传输接上之前这不是缺陷（还没有控制平面这个 sink),所以不在恢复边这批里修,
但**它是 4.3 的前置条件,不是可选项**。
