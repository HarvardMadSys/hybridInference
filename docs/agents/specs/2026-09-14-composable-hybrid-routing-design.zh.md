...
- 同一物理资源的容量声明一致。**（不在 PR #1454 范围）** 共享容量所有权与"未共享的重复受限池拒绝启用"按 §6.1 延期；当前状态是每个 `RouteWiseRouter` 各自持有自己的 manager，重复受限池既不共享也不在构建期拒绝。在它落地之前，同一受限池不应被多个独立实例同时使用。
...
落实精确 endpoint / 池委托的显式语义、请求上下文和兼容映射；保留现有公共导入和单层行为。**共享资源所有者（注入现有 manager、未覆盖的重复受限池与 worker 组合启动失败）不属于 PR #1454，见 §6.1 与 §1.1。**
