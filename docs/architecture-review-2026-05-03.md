...
- **Observability is gutted across the stack.** [serving/observability/metrics.py](serving/observability/metrics.py) is no-op shims, Prometheus scrape is disabled at [infrastructure/prometheus/prometheus.yml:33-40](infrastructure/prometheus/prometheus.yml#L33), only 3 alerts route (the rest blackhole), and there is no distributed tracing. See [serving/](#serving-fastapi-gateway-adapters-storage-observability) and [infrastructure](#infrastructure--config--scripts-deployment--ops).
...
- **Docker Compose** ([infrastructure/docker/docker-compose.yml:16](infrastructure/docker/docker-compose.yml#L16)): postgres:16, backend, frontend, alertmanager, alert-logger.
...
- Alertmanager 0.31.1 receives alerts from disabled Prometheus ([infrastructure/prometheus/prometheus.yml:33-40](infrastructure/prometheus/prometheus.yml#L33)).
- `alert-logger` appends JSONL to `var/log/alert_history.jsonl`.
- Only 3 alerts routed ([infrastructure/alertmanager/alertmanager.yml:12-14](infrastructure/alertmanager/alertmanager.yml#L12)): `"ServiceDown|ServiceUnreachable|DatabaseDisconnected"`; rest blackholed.
...
1. **Monitoring gap**: Prometheus metrics endpoint disabled ([infrastructure/prometheus/prometheus.yml:33-40](infrastructure/prometheus/prometheus.yml#L33)), alerting local-only (blackhole default, 3 hard-coded alerts), no dashboards or external visibility. Production incidents invisible until human reports.
...
- Prometheus scrape disabled at [infrastructure/prometheus/prometheus.yml:33-40](infrastructure/prometheus/prometheus.yml#L33).
- Only 3 alerts route through Alertmanager ([infrastructure/alertmanager/alertmanager.yml:12-14](infrastructure/alertmanager/alertmanager.yml#L12)); rest blackholed.
