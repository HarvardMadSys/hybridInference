...
- Removing the end-user `scripts/setup_claude_code.sh` (configures the local Claude Code CLI to point at `freeinference.org/anthropic`) or any user-facing documentation that advertises freeinference as a Claude Code endpoint.
...
| `scripts/import_codex_auth.py` | `routing/routers.py` (rewrite stale comment) | `scripts/setup_claude_code.sh` |
...
| `freeinference-harness/configs/fixtures/tools-codex-3.yaml` | `frontend/.../playground/page.tsx` (drop reasoning UI) | `docs/superpowers/plans/*` (history) |
...
| File | Specific change |
|---|---|
| `docs/free_inference/README.md` | Drop the Codex bullet (line 17) and the `~/.codex/config.toml` setup step (line 35 onward — likely a multi-line block). |
| `docs/free_inference/docs/source/integrations.md` | Strip the entire Codex section (lines 9–18 intro + config example, plus troubleshooting bullets at 230–231). If Codex was the only integration in the file, leave a stub with surviving integrations or delete the file (verify when editing). |
| `docs/free_inference/docs/source/quickstart.md` line 24 | Drop the Codex `config.toml` setup step. |
| `docs/free_inference/docs/source/models.md` | Delete the `gpt-5.4` model entry (line 161 onward) and the "Codex: Edit `~/.codex/config.toml`" client-config block (line 202 onward). |
| `docs/source/developer/architecture.md` | Drop the diagram row mentioning `codex_token.py` (line 101); drop the `CodexSubscriptionAdapter` table row (line 120); rewrite the AccountPool paragraph (line 122) — there is no shared pool anymore. Drop the `ClaudeSubscriptionAdapter` row if present (verify when editing). |
| `docs/source/developer/configuration.md` | Drop `codex_sub` and `claude_sub` from the supported `kind` list (line 80); delete the Codex-subscription paragraph (lines 300–310) and the `CODEX_ACCOUNTS_FILE` env mention. Drop equivalent Claude-subscription content if present. |
| `docs/source/developer/adding-models.md` | Drop `codex_sub` and `claude_sub` from the supported-providers list (line 295) and the kind reference table (line 435). |
| `docs/source/developer/openrouter.md` line 18 | Drop the `codex_sub.py` reference from the code-tree comment block. |
| `docs/source/developer/deployment.md` | Drop the host-script paragraph mentioning `~/.codex/`, `~/.claude/`, `var/data/` accounts files (line 107) and the `codex --login` / `import_codex_auth.py` instructions (lines 119–120). Verify any `import_claude_auth.py` mention is removed too. |

### Untouched (verified, intentional)

- `docs/superpowers/plans/2026-05-02-*.md`, `docs/superpowers/specs/2026-05-02-*.md` — historical artifacts, frozen in time.
