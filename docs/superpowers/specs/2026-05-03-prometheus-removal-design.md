...
- Prometheus scrape is disabled at [infrastructure/prometheus/prometheus.yml:33-40](../../../infrastructure/prometheus/prometheus.yml#L33-L40); rule files were dropped in commit `3d1721a`.
...
- [infrastructure/prometheus/](../../../infrastructure/prometheus/) — entire directory
- [infrastructure/alertmanager/](../../../infrastructure/alertmanager/) — entire directory (includes `alert_logger.py`)
- Any `alertmanager.service` / `alert-logger.service` units in [infrastructure/systemd/](../../../infrastructure/systemd/)

### Files modified

- `infrastructure/docker/docker-compose.yml` — drop `prometheus`, `alertmanager`, `alert-logger` services + their volumes.
- `infrastructure/nginx/freeinference.conf` — remove metrics-endpoint stanzas if any.
...
- Docs in `docs/source/` and `docs/source/developer/`.
...
4. **PR 4 — Strip Prometheus + Alertmanager infra.** Delete [infrastructure/prometheus/](../../../infrastructure/prometheus/) + [infrastructure/alertmanager/](../../../infrastructure/alertmanager/); remove `docker-compose` services; update env, nginx, systemd, docs.
