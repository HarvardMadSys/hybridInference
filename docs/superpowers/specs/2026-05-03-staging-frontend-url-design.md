...
The backend reads `.env` via `env_file: ../../.env` in `infrastructure/docker/docker-compose.yml`, so a container restart is required for the new value to take effect.
